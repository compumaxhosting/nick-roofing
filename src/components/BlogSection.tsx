    ...
    <section className="max-w-7xl mx-auto py-14 px-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {BlogsData.map((blog, index) => (
          <div
            key={index}
            className="group p-4 bg-white shadow-lg rounded-lg hover:shadow-xl transition-shadow duration-300"
          >
            {/* Blog Image */}
            <Link
              href={`/blog/${blog.slug}`}
            ...
            <div className="mt-4">
              ...
              <Link
                href={`/blog/${blog.slug}`}
                className="block text-xl md:text-xl font-semibold text-[#364f4f] mb-2 group-hover:text-[#1f3d3d] transition-colors"
              >
                {blog.title}
              </Link>
